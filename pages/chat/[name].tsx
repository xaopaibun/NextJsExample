...
import { HandleTime } from '../../src/untils/untils';



const Chat: React.FC = () => {
    const socketRef = useRef<any>();
    const [mess, setMess] = useState<Array<any>>([]);
    ...
    React.useEffect(() => {
        ...
        socketRef.current.on('sendId', (data : string) => {
        ...
        socketRef.current.on('OffTyping', (data : string)=> {
            setTyping(data)
        })

        socketRef.current.on('sendDataServer', (dataGot: Array<any>) => {
          console.log(dataGot)
            setMess(dataGot)
        })

        socketRef.current.on('sever_send_newuser', (user: string) => {
